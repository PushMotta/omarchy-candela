...
sandbox2="$(new_sandbox)"
run_cli "$sandbox2" apply '{"displays":[{"name":"DP-2","bitdepth":10,"cm":"hdr","sdr_max_luminance":203}]}' >/dev/null
[[ -s $sandbox2/state/displays-pending.lua ]] || fail "pending toggles file written on a timed apply"
assert_contains "$(cat "$sandbox2/state/displays-pending.lua")" 'output = "DP-2", mode = "3840x2560@59.98", position = "2400x0", scale = 1.6, transform = 0, bitdepth = 10, cm = "hdr", sdr_max_luminance = 203' "pending file carries the change"
assert_contains "$(head -1 "$sandbox2/state/displays-pending.lua")" "Generated by" "pending file carries the header"
...
run_cli "$sandbox2" revert >/dev/null
[[ ! -e $sandbox2/state/displays-pending.lua ]] || fail "revert removes the pending toggles file"
...
[[ ! -e $sandbox2/state/displays-pending.lua ]] || fail "keep removes the pending toggles file"
...
[[ ! -e $sandbox2/state/displays-pending.lua ]] || fail "no pending toggles file survives a failed readback"
...
assert_contains "$(cat "$sandbox2/state/displays-layout.lua")" 'omarchy_displays_layout_probe = "' "layout carries a load probe"
...
assert_contains "$(cat "$sandbox2/state/displays-layout.lua")" 'output = "DP-2", mode = "3840x2560@59.98", position = "2400x0", scale = 1.6' "a re-enabled display returns to its old place, not 0x0"
...
assert_not_contains "$(cat "$sandbox2/state/displays-layout.lua")" 'output = "eDP-1", disabled' "layout carries no disable rule of its own for the panel"
assert_contains "$(cat "$sandbox2/state/displays-layout.lua")" "internal-monitor-disable" "layout says where the panel's off state lives"
...
assert_contains "$(cat "$sandbox2/state/displays-layout.lua")" 'output = "eDP-1", mode = "2880x1800@60", position = "0x0", scale = 2' "the re-enabled panel has a full rule again"
